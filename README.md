...
These controls expose the underlying light properties and all are animatable from within Lightdesk thanks to Blender's RNA system.
...
## Known Issues
1. The re-ordering of panels is not currently tracked or persisted in the scene data or the `.blend file`. The next time your project is loaded the channels will be recreated in the order that they were originally assigned to Lightdesk, not the display order they were in when the `.blend` file was saved. The means of doing this are currently beyond me, but if anyone can figure out how to capture re-ordering events or expose that data via the current Blender API then this should be relatively trivial to achieve. Let me know if you have any idea how to do this.
